...
# About
It uses docker files for deployment via docker. docker-compose file names must be `docker-compose.dev` or `docker-compose.prod` or `docker-compose.qa` with yaml and yml extension without this it fallbacks to `docker-compose.yml` or `docker-compose.yaml`. May provide env file inside the folder that requires rebuild and restart otherwise can provide the env path.
